...
//This empty array is to save the results of the next sequence function
var gamePattern = [];
...
//You'll need a way to keep track of whether if the game has started or not, so you only call nextSequence() on the first click.
var started = false;
...
$(".start").click(function() {
  if (!started) {
    //The h1 title starts out saying "Click any where", when the game has started, change this to say "Level 0".
    $("#level-title").text("Level " + level);
    nextSequence();
    started = true;
    $(".start").hide(); //Hide the button after the game starts
  }
});
...
function checkAnswer(currentLevel) {
  ...
  if (gamePattern[currentLevel] === userClickedPattern[currentLevel]) {
    ...
    $("#level-title").text("Game Over, Press Restart to Rock Again!");
    //You want to change the text of the button to Restart
    $(".start").text("Restart");
...
function nextSequence() {
  ...
  playSound(randomChosenColour);
          ...
          //How the music is played each time
function playSound(name) {
  var audio = new Audio("sounds/" + name + ".mp3");
  audio.play();
}

            //How the animation works each time
